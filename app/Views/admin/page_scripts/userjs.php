<script>
    $(document).ready(function () {
        ...
        table = $('#userTable').DataTable({
            ...
            // ✅ Updated dom layout: search + pagination right aligned
        ...
        $('#confirm-delete-btn').off('click').on('click', function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 1) {
                        alertBox.removeClass('d-none alert-danger').addClass('alert-success').text(response.message).show();
                        table.ajax.reload(null, false); // ✅ Refresh table
                    } else {
                        alertBox.removeClass('d-none alert-success').addClass('alert-danger').text(response.message).show();
                    }
                    setTimeout(() => alertBox.fadeOut(), 2000);
                },
                error: function () {
                    $('#confirmDeleteModal').modal('hide');
                    alertBox.removeClass('d-none alert-success').addClass('alert-danger').text('Something went wrong!').show();
                    setTimeout(() => alertBox.fadeOut(), 2000);
                }
            });
        });
